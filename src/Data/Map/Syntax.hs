...
------------------------------------------------------------------------------
err :: Show a => a -> b
err k = error $ "MapSyntax: Key "++show k++" already exists in the map"
...
runMapSyntax
    ...
    -> MapSyntaxM k v a
    -> map
runMapSyntax alreadyIn forceIns ms =
    foldl f mempty $ execState (unMapSyntax ms) mempty
  where
    f accum ir@ItemRep{..}
      | irKey `alreadyIn` accum = replace accum ir
      | otherwise = forceIns irKey irVal accum

    replace accum ItemRep{..} =
      case irStrat of
        Replace -> forceIns irKey irVal accum
        Ignore -> accum
        Error -> err irKey
